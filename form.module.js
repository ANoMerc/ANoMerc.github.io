...
const FormModule = (() => {
  ...
  // Валидация — достаточно наличия хотя бы одного человека в команде
  const validateForm = () => {
    const calcBtn = document.getElementById('calcBtn');
    const roles   = getTeamRoles();
    const valid   = roles.reduce((s, r) => s + r.count, 0) > 0;
    if (calcBtn) calcBtn.disabled = false; // always enabled
    return true;
  };
  ...
  const displayResults = (inputs, result) => {
    ...
    setText('effortRange',   `~${result.effort} часов трудозатрат · база без резервов: ${result.baseDays} дн.`);
    ...
    // Показываем похожие проекты
    renderSimilarProjects(result.similarProjects || []);
  ...
  const renderSimilarProjects = (projects) => {
    const container = document.getElementById('similarProjectsList');
    if (!container) return;
    if (!projects.length) {
      container.innerHTML = '<p class="form-hint">Похожих проектов в базе не найдено — используется эвристическая оценка.</p>';
      return;
    }
    const methodLabel = { waterfall:'Waterfall', scrum:'Scrum', kanban:'Kanban', safe:'SAFe',
      lean:'Lean', scrumban:'Scrumban', prince2:'PRINCE2', pm2:'PM²', hybrid:'Hybrid' };
    container.innerHTML = projects.map(p => `
      <div class="similar-proj-row">
        <div class="similar-proj-info">
          <span class="similar-proj-name">${p.name}</span>
          <span class="similar-proj-meta font-mono">${methodLabel[p.methodology] || p.methodology} · ${p.teamSize} чел. · ${p.duration_days} дн.</span>
        </div>
        <div class="similar-proj-score">
          <div class="similar-proj-bar">
            <div class="similar-proj-fill" style="width:${Math.round(p._score * 100)}%"></div>
          </div>
          <span class="font-mono" style="font-size:9px;color:var(--muted)">${Math.round(p._score * 100)}%</span>
        </div>
      </div>
    `).join('');
  };
  ...
  const exportJSON = () => {
    ...
    download(JSON.stringify(window.lastCalc, null, 2), 'application/json', 'ncom-estimate.json');
  };
